...
%dx = boxdim(:,2)/8;
%dy = boxdim(:,1)/8;
%boxdim = [bottom-top, right-left];	% Every single rect dimension

% Increments on x and y for every layer (30 for this example)
dx = [ 64 96 96 128 192 32 48 48 64 96 16 24 24 32 48 8 12 12 16 24 4 6 6 8 12 2 3 3 4 6 ];
dy = [ 32 32 64 64 96 16 16 32 32 48 8 8 16 16 24 4 4 8 8 12 2 2 4 4 6 1 1 2 2 3];
Nlayers = size(dx, 2);	% Number of layers (9 or 6 for our text detector parameters so far)

% Computing boxdim
boxdim = zeros(Nlayers, 2);
for indlay = 0:Nlayers-1
	idrect = find(layer==indlay, 'first')
	if size(idrect)>0
		boxdim(indlay, 1) = bottom(indlay) - top(indlay);
		boxdim(indlay, 2) = right(indlay) - left(indlay);
	end
end
cut = max(floor(boxdim(:,1)/8), 1); % Every single rect value to cut from the top and bottom for the resulting boxes after grouping for every layer
